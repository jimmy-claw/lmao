{
  description = "logos-dashboard-plugin — Logos Core IComponent monitoring dashboard for LMAO";
  ...
  outputs = { self, logos-module-builder, nixpkgs }:
    ...
    {
      packages.${system} = {
        # The dashboard plugin library (C++ IComponent).
        lib = pkgs.stdenv.mkDerivation {
            pname = "lmao-dashboard-plugin";
            version = "0.1.0";
            src = ./.;
