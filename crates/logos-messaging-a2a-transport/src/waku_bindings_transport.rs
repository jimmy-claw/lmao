...
impl NativeWakuTransport {
    ...
    pub async fn new(config: WakuNodeConfig) -> Result<Self> {
        ...
        // Set up the event callback to dispatch messages to subscribers
        ...
        node.relay_subscribe(&PubsubTopic::new(DEFAULT_PUBSUB_TOPIC))
            .await
...
///
/// Uses the convention: /a2a/1/{topic}/proto
fn make_content_topic(topic: &str) -> WakuContentTopic {
    WakuContentTopic::new("a2a", "1", topic, Encoding::Proto)
}
